...
import PostUserCard from "../Home/PostUserCard";

const CardHeader = ({post, postaction, setPostaction, serverPublic, users, auth, children}) => {
            ...
            <div className={"PostHeader PostHeader--legacy PostHeader--inPost js-PostHeader"}>
                {
                    users.map((user, id)=>{
                        if(user._id !==auth._id)
                            return <PostUserCard person={user} key={id} post={post} serverPublic={serverPublic}/>
                    })
                }
                {
                    auth.user._id === post.user._id &&
                    <>
                        <div className={"PostHeaderActions PostHeaderActions--inPost PostHeaderActions--legacy"} key={post._id}>
                            <div className={"PostHeaderActions__action"}>
                                <div className={"PostHeaderActionsUiActionMenu"}>
                                    <div className={postaction?"ui_actions_menu_wrap _ui_menu_wrap shown":"ui_actions_menu_wrap _ui_menu_wrap"} onMouseOver={()=>setPostaction(true)} onMouseOut={()=>setPostaction(false)}>
